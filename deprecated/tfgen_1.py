...
class SpatialTemporalHawkes(object):
    ...
    def __init__(self, C=1., maximum=1e+3):
        """
        """
        INIT_PARAM   = 5e-2
        ...
        self.beta    = tf.get_variable(name="beta", initializer=tf.constant(5.), dtype=tf.float32)
    ...
    def sampling(self, T, S, batch_size, keep_latest_k):
        """
    ...
    def log_conditional_pdf(self, points, S, keep_latest_k=None):
        ...
        log_cond_pdf = tf.cond(tf.less(n_points, 2), 
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        hawkes = SpatialTemporalHawkes(C=1., maximum=1e+3)

        init_op = tf.global_variables_initializer()
        sess.run(init_op)

        seqs, logliks = hawkes.sampling(T=[0., 10.], S=[[-1., 1.], [-1., 1.]], batch_size=3, keep_latest_k=None)
